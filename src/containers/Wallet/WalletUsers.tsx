...
import { formatNumber, getWalletStatusBadge } from "./walletUtils";

interface WalletUsersProps {
  onUserSelect?: (userId: number) => void;
}

export function WalletUsers({ onUserSelect }: WalletUsersProps) {
  ...
  const handleWalletClick = (walletId: number) => {
    if (onUserSelect) {
      onUserSelect(walletId);
    } else {
      publish("show-right-panel", walletId.toString());
    }
  };
        ...
        <Table horizontalScrollWithStickyColumns>
          <TableHead>
            <TableRow className="bg-white">
              ...
              <TableHeaderCell align="right" className="w-[200px] shrink-0">
            ...
            {walletUsers.map((user) => (
              ...
              >
                ...
                <TableCell align="left" data-sticky="right">
                  {getWalletStatusBadge(user.status)}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
